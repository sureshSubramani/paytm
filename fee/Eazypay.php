...
class Eazypay{
    ...
    //const DEFAULT_BASE_URL = 'https://eazypayuat.icicibank.com/EazyPG?'; // testing
	const DEFAULT_BASE_URL = 'https://eazypay.icicibank.com/EazyPG?'; // live
    ...
    public function __construct(){
        ...
        $this->paymode                  =    '9';
        $this->return_url               =    'https://mahendra.org/online_payment/fee/TxnResponse.php';
	    //$this->return_url               =    'https://mahendra.org/eazypay/TxnResponse.php';
		
		$url = trim(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH), '/');

        if(isset($_SESSION['MERCHANT_KEY']) || isset($_SESSION['CALLBACK_URL']) ){
            unset($_SESSION['MERCHANT_KEY']);
            unset($_SESSION['CALLBACK_URL']);
            $_SESSION['MERCHANT_KEY'] = !empty($mkey)?$mkey:'';
            $_SESSION['CALLBACK_URL'] = !empty($url)?$url:'';
        }else{
            $_SESSION['MERCHANT_KEY'] = !empty($mkey)?$mkey:'';
            $_SESSION['CALLBACK_URL'] = !empty($url)?$url:'';
        }
    ...
    protected function generateUrl($mandatoryField, $optionalField, $amount, $reference_no){
        $encryptedUrl = self::DEFAULT_BASE_URL."merchantid=".$this->merchant_id."&mandatory fields=".$mandatoryField."&optional fields=".$optionalField."&returnurl=".$this->return_url."&Reference No=".$reference_no."&submerchantid=".$this->sub_merchant_id."&transaction amount=".$amount."&paymode=".$this->paymode;
