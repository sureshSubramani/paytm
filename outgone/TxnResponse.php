...
function payment_success_exec($data) {
	...
	if(isset($_REQUEST) && isset($data['Transaction_Amount']) && isset($data['Response_Code'])) {
        ...
        $qry1 = "SELECT college FROM out_student_payments WHERE order_id like '%$ref_no%' AND record_status=1";
		
        $r = $con->query($qry1);	
        $res = $r->fetch_assoc();

        $qry2 = "SELECT merchant_key FROM `gateway_merchent_details` WHERE college='".$res['college']."' AND gateway='ICICI' AND status=1";
        $isRecords = $con->query($qry2);	
        $eazyPay = $isRecords->fetch_assoc();
 
        if(isset($eazyPay['merchant_key']) && isset($res['college'])){
			...
			if(!empty($data['ReferenceNo'])){
				...
				if ($conn->connect_error) {
				  $update_message = "Connection failed. Unable to update payment details.";
					...
					$sql = "SELECT * FROM out_student_payments WHERE order_id like '%".$ORDERID."%' AND record_status=1";
					...
					if ($result->num_rows > 0) {	
						...
						$sql = "UPDATE `out_student_payments` SET `txnid`=$TXNID, `banktxnid`=$BANKTXNID, `orderid`=$ORDER_ID, `txnamount`=$TXNAMOUNT, `respcode`=$RESPCODE, `mid`=$MID, `paymentmode`=$PAYMENTMODE, `txndate`=$TXNDATE WHERE `order_id` like '%".$ORDERID."%'";

						$result2 = $conn->query($sql);
	
						if($result2){

						$roll_no = $result2['cust_id'];

						$col = "SELECT * FROM out_student_details WHERE `reg_no` = '".$roll_no."' OR `roll_no` = '".$roll_no."' OR `name` = '".$roll_no."' AND status=1";
...
<?php 
		...
		if(!empty($data['ReferenceNo'])){
			...
			if ($result->num_rows > 0) {	
				$sql = "UPDATE `out_student_payments` SET `orderid`=$ORDER_ID, `respcode`=$RESPCODE, `mid`=$MID, `status`='$response' WHERE `order_id` LIKE '%".$ORDERID."%'";
				$result2 = $conn->query($sql);
			}
		}	
	}
	...
	echo "<p> Click <a href='https://mahendra.org/online_payment/outgone/'><i class='fa fa-home'></i> here</a> to return to the home page</p></div>";
?>
<?php endif; ?>
</div>
</body>
