...
function payment_success_exec($data) {
	...
	if(isset($_REQUEST) && isset($data['Transaction_Amount']) && isset($data['Response_Code'])) {
			...
			$sql = "SELECT college_id FROM student_payment WHERE order_id like '%$ref_no%' AND record_status=1";
			$r 	 = $con->query($sql);		
			$res = $r->fetch_assoc();

			if(isset($res['college_id'])){
				$qry = "SELECT merchant_key FROM `gateway_merchent_details` WHERE college='".$res['college_id']."' AND gateway='ICICI' AND status=1";
				$isRecords = $con->query($qry);	
				$eazyPay = $isRecords->fetch_assoc();				
			}
				
			if(isset($eazyPay['merchant_key'])){
...
<body>
    <div class="container">
		...
		$code = response_code($data['Response_Code']);
		header('Location:payment_status.php?status_code='.$code);
 		else: 
		...
		if ($conn->connect_error) {
			...
			if ($result->num_rows > 0) {	
				$sql = "UPDATE `student_payment` SET `orderid`=$ORDER_ID, `respcode`=$RESPCODE, `mid`=$MID, `status`='$response' WHERE order_id LIKE '%".$ORDERID."%'";
				$result2 = $conn->query($sql);
			}
		}	
	}
	...
	echo "<p> Click <a href='https://mahendra.org/online_payment/fee/'><i class='fa fa-home'></i> here</a> to return to the home page</p></div>";
?>
        <?php endif; ?>
    </div>
</body>
